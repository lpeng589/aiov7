...
<input type="hidden" name="type" id="type" value="fieldScope"/>
<input type="hidden" name="readDeptIds" id="readDeptIds" value="$!fieldScopeBean.readDeptIds"/>
<input type="hidden" name="readUserIds" id="readUserIds" value="$!fieldScopeBean.readUserIds"/>
<input type="hidden" name="hideDeptIds" id="hideDeptIds" value="$!fieldScopeBean.hideDeptIds"/>
<input type="hidden" name="hideUserIds" id="hideUserIds" value="$!fieldScopeBean.hideUserIds"/>
<input type="hidden" name="fieldsName" id="fieldsName" value="$!fieldScopeBean.fieldsName"/>
<input type="hidden" name="tableName" id="tableName" value="$!fieldScopeBean.tableName"/>
<input type="hidden" name="keyId" id="keyId" value="$!fieldScopeBean.id"/>
	<div style="overflow:hidden;padding:5px 0 0 10px;">
		<div style="float:left;display:inline-block;line-height:26px;">当前位置：字段权限添加</div>
		<div class="btn btn-small" onclick="beforeSubmit()" style="float:right;">更新</div>
	</div>
					...
					<div class="slt_wdv">
						<div class="bk_dv">
							<select class="left_slt" multiple name="left" id="left" size="8" style='width:100px;' ondblclick="moveOption(document.getElementById('left'), document.getElementById('right'))">
							   #set($selectFields = ","+$fieldScopeBean.fieldsName)
						...
						<div class="bk_dv">
							<select class="right_slt" multiple name="right" id="right" size="8" style='width:100px;' ondblclick="moveOption(document.getElementById('right'), document.getElementById('left'))">
								#foreach($fieldName in $fieldScopeBean.fieldsName.split(","))
										...
										<option title="$fieldName" value="$fieldBean.fieldName">明细--$fieldBean.display.get("$globals.getLocale()")</option>
									#end
								#end
							</select>
					...
					<div class="slt_woy">
						<div class="woy_lt">
							<select class="slt" size="5" name="readDeptName" id="readDeptName" multiple="multiple">
								#foreach($deptId in $fieldScopeBean.readDeptIds.split(","))
									#if("$!deptId" !="")
									<option value="$deptId">
										$deptMap.get("$deptId")
									</option>
					...
					<div class="slt_woy">
						<div class="woy_lt">
							<select class="slt" size="5" id="readUserName" name="readUserName" multiple="multiple">
								#foreach($userId in $fieldScopeBean.readUserIds.split(","))
									#if("$!userId" !="")
									<option value="$userId">
					...
					<div class="slt_woy">
						<div class="woy_lt">
							<select class="slt" size="5" name="hideDeptName" id="hideDeptName" multiple="multiple">
								#foreach($deptId in $fieldScopeBean.hideDeptIds.split(","))
									#if("$!deptId" !="")
									<option value="$deptId">
										$deptMap.get("$deptId")
									</option>
					...
					<div class="slt_woy">
						<div class="woy_lt">
							<select class="slt" size="5" id="hideUserName" name="hideUserName" multiple="multiple">
								#foreach($userId in $fieldScopeBean.hideUserIds.split(","))
									#if("$!userId" !="")
									<option value="$deptId">
										$!globals.getEmpFullNameByUserId($userId)
									</option>
