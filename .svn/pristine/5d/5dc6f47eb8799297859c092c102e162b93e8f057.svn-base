 ...
 * <p>Title: 实现了DES加解密</p>
 *
 * <p>Description: 加密规则为ECB电子文本加密方式，补位规则为PKCS#5Padding方式</p>
 *
...
public class DES {
    ...
    public byte[] encrypt(byte[] key,byte[] data){
        return this.DesEncrypt(key,data,1);
    }

    public byte[] decrypt(byte[] key, byte[] data) {
        byte[] b = this.DesEncrypt(key, data, 0);
        //按照PKCS#5Padding补位规则丢弃加密时补位的数据
        byte[] ret;
        int paddingLength = 0;
        for(int i=1;i<9;i++){
            boolean equal = true;
        ...
        ret = new byte[b.length - paddingLength];
...
//
//        DES1 des = new DES1();
//        byte[] result = des.DesEncrypt(key.getBytes(), data.getBytes(), 1);
//
//        System.out.println( "source :" +MsgHeader.bytes2hex(key.getBytes()));
//        System.out.println( "encrypt :" +MsgHeader.bytes2hex(result));
//
//        byte[] result2 = des.DesEncrypt(key.getBytes(), result, 0);
//        System.out.println( "decrypt :" +MsgHeader.bytes2hex(result2));
        int bytelen = data.getBytes().length;
        byte[] result = new byte[(bytelen + 8) - (bytelen % 8)];
        byte[] bytekey = key.getBytes();
        byte[] bytedata = data.getBytes();
        System.out.println( MsgHeader.bytes2hex(bytedata));
...
//        }
        System.out.println();
        DES des = new DES();
        result = des.encrypt(bytekey, bytedata);
        System.out.println( MsgHeader.bytes2hex(result));
//        for (int i = 0; i < result.length; i++)
//        {
//            System.out.print(" " + result[i] + " ");
//        }
        System.out.println();
        result = des.decrypt(bytekey, result);
        System.out.println( MsgHeader.bytes2hex(result));
//        for (int i = 0; i < result.length; i++)
//        {
//            System.out.print(" " + result[i] + " ");
