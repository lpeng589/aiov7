...
$.TextboxList = function(element, _options){
	...
	var add = function(plain, id, html, afterEl){
	  if(self.valuebit.indexOf(plain)==-1){
	      self.valuebit=self.valuebit+","+plain;
	  }else{
	      return self;
	  }
	...
	valuestr=self.valuebit.split(",");
	var newstr="";
	for(var i in valuestr){
	   var u=valuestr[i];
	   if(u!=""&& u!=bitstr){
	      newstr=newstr+","+u;
	   }
	}
	self.valuebit=newstr;
		if (!focused) return;
	...
	var init = function(){
		...
		if (type == 'box'){
			...
			element.blur(function(){
			  var lilist=$("#"+textboxlist.element.attr("id")+"_listResult  li ");
				var shu=0;
				var selectli;
				lilist.each(function(index,self){
				     self=$(self);
				      if( self.is("[class='selectli']")){
				         self.click();				        
				      }
				});	
				element.val("");
						 $("#"+textboxlist.element.attr("id")+"_listResult").children().html("");
			});	
			...
			//柯志良  键盘按下事件
				...
				lilist.each(function(index,self){
				  ...
				  if(shu != 0){
				     var a=selectli;
				    var b= a.next();
				    a.removeClass("selectli");
				    b.addClass("selectli");
				  }else{
				  lilist.first().addClass("selectli");
				  }
				  return;
				}
				...
				$("#"+textboxlist.element.attr("id")+"_listResult").click(function(){
				         element.val("");
						 $("#"+textboxlist.element.attr("id")+"_listResult").children().html("");
				});		   		  
				   if (!focused) return;
				   var evStop = function(){ ev.stopPropagation(); ev.preventDefault(); };
					...
					content=encodeURIComponent(content);
					    url="/PublicServlet?operation=ajaxTextboxListEmp&ShowType=json&keyWord="+content;
						jQuery.ajax({
   							type: "GET",
   							url: url,
   							success: function(msg){
       								$("#"+textboxlist.element.attr("id")+"_listResult").children().html("");
					            $("#"+textboxlist.element.attr("id")+"_listResult").children().append(msg);
					             $("#"+textboxlist.element.attr("id")+"_listResult").children().find("li").each(function(){
					                  ...
					                  $(this).click(function(){
					                     		var id=$(this).attr("id");
 												 ...
 												 textboxlist.add("<a class='selectlist' empid='"+id+"' onclick='yy(this)' >"+text+"</a>");
  												$(this).parent().html("");
					                  });
					             });
					              $("#"+textboxlist.element.attr("id")+"_listResult  li").first().addClass("selectli");
   							}
						});
					}else{
					    $("#"+textboxlist.element.attr("id")+"_listResult").children().html("");
