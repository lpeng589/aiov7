...
$(document).ready(function() { //进入列表后马上执行查询
	reportQuery("tzESales",true,{pageSize:pageSize,pageNo:pageNo},function(conditions,cols,rows){
		conds = conditions;
		
		for(i=0;i<conds.length;i++){
			if(conds[i][0]=='agtName'){
				hasAgtName = true; //代理商这个字段有权限控制，没权限的人要隐藏 这里如果查询条件中没有代理商字段表示没权限，从而控制本界面中是否显示代理字段
			}
			if(conds[i][0]=='workFlowNodeName'){ //初始化条件界面中的默认值
				val = conds[i][1]==""?"all":conds[i][1]; 
				$("#QworkFlowNodeName_"+val).attr("checked","checked");
			}else{
	...
	canAdd("tzESales","","",function(data){ //判断系统是否有添加权限,没有要隐藏
		if(data=="true"){
			$("#addBt").show();
		}else{
			$("#addBt").hide();
...
function query(isClear){
	...
	//查询的条件参数
	cond = {BillDate1:$("#QBillDate1").val(),
			workFlowNodeName:$("input[name=QworkFlowNodeName]:checked").val(),
			agtName:$("#QagtName").val(),
	...
	//执行报表查询		 
	reportQuery("tzESales",true,cond,function(conditions,cols,rows){
		addList(cols,rows);
...
function addList(cols,rows){	
	for(i = 0 ;i<rows.length;i++){ 
		...
		$("#listDiv").append( 
		      ...
		      (hasAgtName?'<p>代理商 '+rowDis.agtName+'</p>':'')+
		      '<p>'+rowDis.empFullName+',购买'+rowDis.product+''+rowDis.qty+''+
		      ', 当前'+rowDis.workFlowStatus+'状态</p>'+
		      '</a></li>'
		 ); 
	 }
...
function showAdd(){
	...
	if($("#pageUpdate").find("div").size() == 0){
		//通过ajax把界面调入
		ajaxUpdateHtml("tzESales.jsp","pageUpdate",function(){
			add();
		});
	}else{
		add();
...
<div data-role="page" id="conditon">
  ...
  <div data-role="content">
    <label for="QBillDate1">下单日期大于</label>
	<input type="date" id="QBillDate1" value="">
	<fieldset data-role="controlgroup"  data-type="horizontal">
	<legend>单据状态</legend>
	<label for="QworkFlowNodeName_all">全部</label>
	<input type="radio" name="QworkFlowNodeName" id="QworkFlowNodeName_all" value="all">
	<label for="QworkFlowNodeName_notApprove">审核中</label>
	<input type="radio" name="QworkFlowNodeName" id="QworkFlowNodeName_notApprove" value="notApprove"> 
	<label for="QworkFlowNodeName_finish">审核结束</label>
	<input type="radio" name="QworkFlowNodeName" id="QworkFlowNodeName_finish" value="finish">
	<label for="QworkFlowNodeName_draft">草稿</label>
	<input type="radio" name="QworkFlowNodeName" id="QworkFlowNodeName_draft" value="draft"> 
	</fieldset>
	<label for="QagtName">代理商</label>
	<input type="text" id="QagtName" value="">
	<label for="QclientName">客户</label>
...
</div> <!-- pageUpdate 添加修改界面  这里用于显示修改界面，执行ajax从tzESales.jsp中截取本div中的html代码贴在这里 -->
