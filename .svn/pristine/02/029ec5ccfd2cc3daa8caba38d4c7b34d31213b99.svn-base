...
function checkTitle(obj){
	var oldTitle=jQuery.trim($("#oldVal").val());
	var FORM_HTML = oDOM.body.innerHTML; 
	var title= GetE('txtName').value.Trim();
	if((oldTitle.length>0 && oldTitle!=title) || (oldTitle.length==0)) { //增加或修改操作
		if(!checkSameLabel(FORM_HTML,title)){
			alert("控件{"+title+"}已存在，请重新输入!");
			return false;
		}else{
			$(obj).removeClass("redInput");
		}
	}else{ //复制后修改操作
		var IEtitleStr="title="+title+" ";  //IE浏览器
		var googletitleStr="title=\""+title+"\""; //谷歌浏览器
		if(title==oldTitle){
			if(FORM_HTML.indexOf(IEtitleStr)>-1 && FORM_HTML.lastIndexOf(IEtitleStr)!=FORM_HTML.indexOf(IEtitleStr)){
				alert("控件{"+title+"}已存在，请重新输入!");
				return false;
			}
			if(FORM_HTML.indexOf(googletitleStr)>-1 && FORM_HTML.lastIndexOf(googletitleStr)!=FORM_HTML.indexOf(googletitleStr)){
				alert("控件{"+title+"}已存在，请重新输入!");
				return false;
			}
		}
...
function checkSameLabel(str,value){
	var IEvalue="title="+value;  //IE浏览器
	var googleValue="title=\""+value+"\""; //谷歌浏览器
	var re=/title=.[^title=]*\s{1,}/g;
	var rr=str.match(re);
	if(rr!=null){
		var newStr=rr.toString();
		if(newStr.indexOf(IEvalue)>-1 || newStr.indexOf(googleValue)>-1 ){
			return false;
		}
	}
	return true;
