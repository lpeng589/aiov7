...
{	
	...
	//	alert("$text.get('oa.news.contentLength')") ;
	//	return false ;
	//}
	return true;	
}
...
{
window.location.href="/OACompanyNewsInfo.do?operation=4&newsType=$!newsType1&newsTitle=$globals.encode($!newsTitle1)&proUserName=$globals.encode($!proUserName1)&beginTime=$!beginTime1&endTime=$!endTime1&pageNo=$!pageNo" ;
}
}
...
<body>
<form action="/OACompanyNewsInfo.do?operation=$globals.getOP("OP_UPDATE")&type_upd=upd_newsInfo" onSubmit="return checkForm();" name="form" method="post">
<input type="hidden" name="popedomUserIds" id="popedomUserIds" value="$!globals.get($arr_newsInfo,8)"/>
<input type="hidden" name="popedomDeptIds" id="popedomDeptIds" value="$!globals.get($arr_newsInfo,9)"/>
<!-- 存查询条件传入的参数-->	
<input type="hidden" name="newsType1" id="newsType1" value="$!newsType1"/>
<input type="hidden" name="newsTitle1" id="newsTitle1" value="$!newsTitle1"/>
<input type="hidden" name="proUserName1" id="proUserName1" value="$!proUserName1"/>
<input type="hidden" name="beginTime1" id="beginTime1" value="$!beginTime1"/>
...
<div class="Heading">
	...
	<div class="HeadingTitle">$text.get("oa.common.updateNews")</div>
	<ul class="HeadingButton">
				<li><button type="button" onClick="forback()" class="b2">$text.get("common.lb.back")</button></li>
...
oDiv.style.height=sHeight+"px";
		...
		<tr>
				...
				<td>
					<input type="hidden" name="newsId" value="$!globals.get($arr_newsInfo,0)" />
					<input name="newsTitle" type="text" value="$!globals.get($arr_newsInfo,2)" size="60" class="text"><font color="#FF0000">* </font>
				</td>
			</tr>
		<tr>
			 ...
			 <td><label for="select"></label>
				...
				#foreach($row_newsType in $globals.getEnumerationItems("NewsType"))
		  		<option value="$row_newsType.value" #if($!globals.get($arr_newsInfo,1)==$row_newsType.value) selected="selected" #end>$row_newsType.name</option>
	   			#end
				</select>
		     </td>
		</tr>	
		<tr>
			 ...
			 <td>
			 	<span>
				<textarea name="Notepadcontent" style="width:800px;height:300px;visibility:hidden;">$globals.get($arr_newsInfo,3)</textarea>
				</span>
			 </td>
		</tr>	
		<tr>
			...
			<button name="picbuttonthe222" onClick="openAttachUpload('/news/','PIC');" class="b4">$text.get("upload.lb.picupload")</button>
					...
					#foreach($att in $!globals.get($arr_newsInfo,14).split(",|;"))
					#if($att.length()>0)
					<div  id ="$att" style ="height:18px; position:relative; color:#ff0000;float:left;vertical-align:top">
					<div style=" position:absolute; top:0px; left:0px; float:left; "><a href="javascript:;"  onClick="removeFile('$att','PIC');">
						<img src="/$globals.getStylePath()/images/del.gif"></a></div>&nbsp;&nbsp;&nbsp;&nbsp;
						<a href="/ReadFile.jpg?type=PIC&tempFile=path&path=/news/&fileName=$att"  target="_blank"><img src="/ReadFile.jpg?type=PIC&tempFile=path&path=/news/&fileName=$att" width="150" border="0">
		...
		<tr>
				...
				<td >
					#set ($isUsed = $!globals.get($arr_newsInfo,4))
					#if($isUsed=="1")
					 	#set ($check11="checked")
				    #else 
					 	#set ($check22="checked")
					#end
					<input name="isUsed" type="radio" id="radiobutton" value="1" $!check11> $text.get("oa.common.immediatelyPublish")
					<input type="radio" name="isUsed" value="0" id="radio" $!check22>$text.get("oa.common.sincePublish")
				</td>
		 </tr>
		 <tr>
			 ...
			 <td><!--
		 		#foreach($row_wakeUpType in $globals.getEnumerationItems("WakeUpMode"))
		 			#if($row_wakeUpType.value==$globals.get($wakeUpType,0)||$row_wakeUpType.value==$globals.get($wakeUpType,1)
		 				||$row_wakeUpType.value==$globals.get($wakeUpType,2)||$row_wakeUpType.value==$globals.get($wakeUpType,3))
		 				#set ($check="checked")
		 			#end
		 			<input type="checkbox" name="wakeUpMode" value="$row_wakeUpType.value" $!check/>$row_wakeUpType.name
			...
			<tr>
			 ...
			 #set ($isAlone = $!globals.get($arr_newsInfo,10))
			...
			<td>
			 <input name="isAlonePopedmon" onClick="chikeRadio(this)" type="radio" $!check1  value="0" id="radio" >
			  $text.get("oa.common.all")
			<input name="isAlonePopedmon" type="radio" onClick="chikeRadio(this)" $!check2 id="radiobutton" value="1" >
			  $text.get("oa.common.appoint")
			 </td>
			</tr>
		   <tr id="_trno" style="$!style">
				<td class="oabbs_tr" id="_title" style="$!style">&nbsp;</td>
				<td valign="middle">
				<div class="oa_signDocument1" id="_context">
				<div class="oa_signDocument_3"><img src="/$globals.getStylePath()/images/St.gif" onClick="showDept();"  alt="$text.get("oa.common.adviceDept")" class="search">&nbsp;
					<a href="javascript:void(0)" title="$text.get("oa.common.adviceDept")" onClick="showDept();">$text.get("oa.common.adviceDept")</a></div>
				...
				<div class="oa_signDocument1" id="_context">
				<div class="oa_signDocument_3"><img src="/$globals.getStylePath()/images/St.gif" onClick="show2();" alt="$text.get("oa.common.advicePerson")" class="search">&nbsp;
					<a href="javascript:void(0)" title="$text.get("oa.common.advicePerson")" onClick="show2();">$text.get("oa.common.advicePerson")</a></div>
				...
				<input type="hidden" name="EmpGroupId" id="EmpGroupId" value="$!globals.get($arr_newsInfo,12)">
						<div class="oa_signDocument1" id="_context">
			...
			<td>
					...
					#set ($iwhetherAgreeReply=$!globals.get($arr_newsInfo,15))
					#if($iwhetherAgreeReply=="1")
						#set ($c1="checked")
					#else
						#set ($c2="checked")
					#end
		   ...
		   <tr>
			...
			<td>
				#set ($isSaveReading=$!globals.get($arr_newsInfo,13))
					#if($isSaveReading=="1")
						#set ($c1="checked")
					#else
						#set ($c2="checked")
			...
			<tr>
			  ...
			  <button type="reset" class="b2" onClick="rt()">$text.get("common.lb.reset")</button></td>
          </tr>
		  </tbody>
		</table>
</div>
