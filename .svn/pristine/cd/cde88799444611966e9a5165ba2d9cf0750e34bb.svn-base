...
function opaction(obj){ 
	...
	curOPObj = obj;
	$(obj).append('<div id="viewOpDiv" class="custom-menu"><ul></ul></div>');
	...
	$("#list_operatoin >a ",document).each(function(){
		...
		if(value.trim()!= "" && value.indexOf("draftAudit")> -1){ 
			trisdraft = obj.parents("tr").attr("draft");			
			if(trisdraft != null && typeof(trisdraft) != "undefined" && trisdraft == "true"){				
		...
		}else if(value.trim()!= "" && value.indexOf("exportAll()")==-1 && value.indexOf("deleteList")==-1 ){
			opactionaddone(name+":javascript:void(0);opSetSelect('"+vid+"');"+value);
		}
	});
	...
	//ftop =obj.offset().top+5;
	//fleft =obj.offset().left+5;
	//alert(ftop+":"+fleft);
	//$("#viewOpDiv",document).css("left",fleft+"px"); 
	//$("#viewOpDiv",document).css("top",ftop+"px");
...
$(function(){	
	...
	$(".opbt").hover(
		function(){
			opaction($(this));
			var oHegiht = $(this).find(".custom-menu").height();
			var offHiehgt = $(this).offset().top;
			var wHeight = $(document).height()-35;
			if((oHegiht+offHiehgt)>wHeight){
				$(this).find(".custom-menu").css("top",wHeight-(oHegiht+offHiehgt));
			}
	...
	$(".opbt",document).mouseover(function(){ 
		opaction($(this));
	}).mouseout( function() { $("#viewOpDiv",document).remove(); } );
	*/
});
