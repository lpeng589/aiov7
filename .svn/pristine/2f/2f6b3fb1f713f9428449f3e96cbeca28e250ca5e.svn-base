...
Calendar.prototype = {
  ...
  Draw: function() {
	...
	if(arr.length>35){
		$(".Calendar").css("background","url(/style/images/plan/beijing2.gif)").css("height","264") ;
	}else{
		$(".Calendar").css("background","url(/style/images/plan/beijing.gif)").css("height","230") ;
	}
	var dHeight = $(".Calendar").height() ;
	var sHeight = document.documentElement.clientHeight-dHeight-130;
	$("#treeNode").css("height",sHeight);
	while(arr.length){
		//每个星期插入一个tr
		...
		//每个星期有7天		for(var i = 1; i <= 7; i++){
			...
			if(arr.length){
				...
				if(d){
					...
					cell.innerHTML = "<a href='javascript:void(0)' onclick='changeColor(this);clickDate(\""+(this.Year+"-"+varMonth+"-"+varDay)+"\")'>"+d+"</a><span id='b"+varDay+"'></span>";
					this.Days[d] = cell;
