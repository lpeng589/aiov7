...
public class HuiCongInformationFinder implements InformationFinder {
	private static final String EMAIL_PATTERN = "\\w+([-+.]\\w+)*@\\w+([-.]\\w+)*\\.\\w+([-.]\\w+)*";

	private static final String QQ_PATTERN = "(QQ|qq)[\\D]*[1-9][0-9]+";

	private static final String PHONE_PATTERN = "电话[:：]([\\d- ]+)";

	private static final String PHONE_PATTERN2 = "[\\D]((0[0-9]+-)?[1-9][0-9]{6,7})[\\D]";

	private static final String MOBILE_PATTERN = "移动电话[:：]([\\d- ]+)";

	private static final String FAX_PATTERN = "传真[:：]([\\d- ]+)";
