...
<div class="col">
	<span>审核状态：</span>
	<ul class="postpayStatus" >
		#set($telecomRecordVal = $!conMap.get('telecomRecordVal'))
		<input type="hidden" id="telecomRecordVal" name="telecomRecordVal" value="$!telecomRecordVal"/>
		<li #if("$!telecomRecordVal" == "all" || "$!telecomRecordVal" == "") class="sel" #end statusVal="all"><a>全部</a></li>
		<li #if("$telecomRecordVal" == "empty") class="sel" #end statusVal="empty" title="未录取"><a>未录取</a></li>
		<li #if("$telecomRecordVal" == "approveing") class="sel" #end statusVal="approveing" title="录取审核中"><a>录取审核中</a></li>
		<li #if("$telecomRecordVal" == "finish") class="sel" #end statusVal="finish" title="录取已审核"><a>录取已审核</a></li>
		<!-- 
...
#foreach($col in $moduleViewBean.listFields.split(","))
		...
		#set($field=$globals.getField($col,$clientTableName,$contactTableName))
			...
			#end
		#else
			#set($minfoTranFields = $minfoTranFields + ${col}+"," )	
		#end
	#end
#end
<!-- 
...
#set($shareOp = "true")
#end
<ul class="col  context-menu-one" id="scroll$!client.get('id')" shareOp="$shareOp" clientName="$client.get('ClientName')">
...
<div class="moperate">
	...
	#end
	#if($LoginBean.operationMap.get("/OACalendarAction.do?crmEnter=true").query())
