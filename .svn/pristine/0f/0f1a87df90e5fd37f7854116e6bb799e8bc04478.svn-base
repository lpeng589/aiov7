...
function addVote(){
	#set($count=0)
	#foreach($voteAnswer in $voteBean.voteAnswer)
		#set($count=$count+1)
		#if($count<$voteBean.voteAnswer.size())
			addMore();
		#end
	#end
	...
	#set($str="")
	#foreach($voteAnswer in $voteBean.voteAnswer)
		$("input[name='voteAnswers']:eq($num)").val("$!voteAnswer.voteAnswer");
		#set($num=$num+1)
	#end
}
function isExist(checkvalue){
	...
	<body onload="addVote();">
		<form action="/OABBSVoteAction.do?type=upvote" name="form" onSubmit="return checkForm()" method="post">
			...
			<div class="Heading">
				...
				<div class="HeadingTitle">
					$text.get("oabbs.com.title.IssueVote")
				</div>
				<div class="HeadingButton">
					...
					<button type="button" class="b2"onclick="location.href='/OABBSForumAction.do?operation=70&forumId=$!forum.id&topicId=$!forum.topic.id&pageNo=$!pageNo&searchSel=$!searchSel'; ">
						...
						<thead>
							<tr>
								...
								<td colspan="2" align="center">
									$text.get("mrp.lb.update")$text.get("oabbs.com.title.Vote")
								</td>
							</tr>
						</thead>
						<tbody>
							<tr>
							<!-- 
								<td rowspan="9" valign="top" class="oabbs_lefttdbg">
							 ...
							 -->
								<td class="oabbs_tr" width="10%">
									$text.get("oabbs.com.lb.IssueTheme")：&nbsp;&nbsp;
								</td>
								<td width="80%">
									<input name="voteTopic" type="text" class="text"
										style="width:355px;" value="$voteBean.voteTopic">
							...
							<tr>
								<td class="oabbs_tr">
									$text.get("oabbs.com.lb.IssueExplain")：&nbsp;&nbsp;
								</td>
								<td>
									<textarea name=voteRemark
										style="width:90%;height:300px;visibility:hidden;">$voteBean.voteRemark</textarea>
								</td>
							</tr>
							<tr>
								<td class="oabbs_tr">
									$text.get("oabbs.com.lb.IssueForm")：&nbsp;&nbsp;
								</td>
								<td>
									<input type="radio" name="voteType" #if($!voteBean.voteType==
										"0")checked="checked" #end value="0" />
									$text.get("oa.inquisition.anonymity")
									<input type="radio" name="voteType" #if($!voteBean.voteType==
										"1")checked="checked" #end value="1" />
									$text.get("oa.inquisition.onymous")
								</td>
							</tr>
							<tr>
								<td class="oabbs_tr">
									$text.get("alertCenter.lb.startDate")≥：&nbsp;&nbsp;
								</td>
								<td>
									<input name="beginTime" value="$voteBean.beginTime"
										onKeyDown="if(event.keyCode==13) openInputDate(this);"
										onClick="openInputDate(this);" class="text">
									<font color="#FF0000">* </font>
								</td>
							</tr>
							<tr>
								<td class="oabbs_tr">
									$text.get("alertCenter.lb.endDate")≤：&nbsp;&nbsp;
								</td>
								<td>
									<input name="endTime" value="$voteBean.endTime"
										onKeyDown="if(event.keyCode==13) openInputDate(this);"
										onClick="openInputDate(this);" class="text">
									<font color="#FF0000">* </font>
								</td>
							...
							<tr>
								<td class="oabbs_tr">
									$text.get("oa.inquisition.answerType")：&nbsp;&nbsp;
								</td>
								<td>
									#foreach($row in $globals.getEnumerationItems("answerType"))
									<input type="radio" name="answerType" value="$row.value"
										#if($!voteBean.answerType==$row.value) checked="checked" #end />
									$row.name #end
								</td>
							</tr>
							<tr id="mytr">
								<td class="oabbs_tr">
									&nbsp;$text.get("oa.inquisition.option")：&nbsp;&nbsp;
								</td>
								<td>
									<div id="type_dx">

										<div id="more">
											<input name="voteAnswers" id="voteAnswers" class="text"
												onkeyup="textValue(this);">
											<img src="/$globals.getStylePath()/images/add.gif"
												onClick="addMore()">
										</div>

									</div>
									<div id="type_wb" style="display:none;">
									</div>
								</td>
							...
							<tr id="mytr">
								...
								<td>
									<div id="type_dx">
										<div id="more">
											<input type="checkbox" name="replayNote" value="1">
											$text.get("bbs.lb.replayNote")
										</div>
									</div>
