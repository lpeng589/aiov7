...
function addmain(){
	...
	asyncbox.open({
 		id:'AddUpdatediv',
		title : '[角色:$!roleName]&nbsp;&nbsp;$!{tableDisplay}管辖范围-添加',
		url :"/RoleAction.do?roleId=$roleId&roleName=$!globals.encode($!roleName)&change=yes&operation=$globals.getOP("OP_SCOPE_RIGHT_ADD_PREPARE")&winCurIndex=$!winCurIndex&type=$!type&tableName=$!tableName&fromUser=$!fromUser",
		width : 800,
		height :500,
...
function updatemain(keyId){
	...
	asyncbox.open({
 		id:'AddUpdatediv',
		title : '[角色:$!roleName]&nbsp;&nbsp;$!{tableDisplay}管辖范围-修改',
		url :"/RoleAction.do?operation=$globals.getOP("OP_SCOPE_RIGHT_UPDATE_PREPARE")&keyId="+keyId+"&roleId=$roleId&roleName=$!globals.encode($!roleName)&winCurIndex=$!winCurIndex&type=$!type&tableName=$!tableName&fromUser=$!fromUser",
		width : 800,
		height :520,
		btnsbar : jQuery.btn.OKCANCEL,
		callback : function(action,opener){
...
oDiv.style.height=sHeight+"px";
	...
	<div id="deptRightTypeDiv3" style="padding-left:5px;">
		...
		<ul class="HeadingButton" style="padding:5px 0;margin:0;">
			<li id="add"  style="display:block;" ><span onClick="addmain();" class="hBtns">$text.get("common.lb.add")</span></li>		
		</ul>
			<table border="0" cellpadding="0" cellspacing="0" class="listRange_list_function" name="table" id="tblSort">
				<thead>
					<tr>
						<td width="200" align="center">$!{tableDisplay}</td>
						<td width="*" align="center">应用于模块</td>
						<!-- <td width="90" align="center">允许添加同级</td>  -->
				...
				<tbody>
					#foreach ($row in $result )
						<tr>
							...
							<td >#if("$!globals.get($row,6)"=="1") 所有模块 #else  指定模块 #end &nbsp;</td>
							<!--<td align="center">#if("$!globals.get($row,9)"=="1") 允许 #else  禁止 #end &nbsp;</td>  -->
							<td class="oabbs_tc">
