...
<body>
	<form method="post" scope="request" name="form"
		action="/FieldScopeSetAction.do?operation=$globals.getOP("OP_ADD")&addType=fieldScopeSet" class="mytable">
		<input type="hidden" name="readpopedomDeptIds" id="readpopedomDeptIds" value="" />
		<input type="hidden" name="readpopedomUserIds" id="readpopedomUserIds" value="" />
		<input type="hidden" name="hiddenpopedomDeptIds" id="hiddenpopedomDeptIds" value="" />
		<input type="hidden" name="hiddenpopedomUserIds" id="hiddenpopedomUserIds" value="" />
											          ...
											          <fieldset>
														...
														<select name='SelectFields' id="SelectFields" multiple="multiple" size="10"  >
														    #foreach($field in $fieldList)
															    #if($globals.canSelectField($field,$!mainTableName,$!childTableName)==true)
														    		#if($field.tableBean.tableName=="$!mainTableName")
																		#set($f=$globals.getFieldBean("$!mainTableName",$field.fieldName))
																		#set($dis=$globals.getLocaleDisplay("$f.display"))
															    		<option value="$field.fieldName">$dis</option> 
														    		#else
														    			#set($f=$globals.getFieldBean("$!childTableName",$field.fieldName))
																		#set($dis=$globals.getLocaleDisplay("$f.display"))
