...
function openSetting(){
	showModalDialog('/CareforDelAction.do?operation=$globals.getOP("OP_UPDATE")&type=setMail&action=query',winObj,'dialogWidth=350px;dialogHeight=320px');
...
function val(){
	...
	if(dateValue==1){
		...
		if(!(isInt(form.runTimes.value) && form.runTimes.value>0)){
			alert("$text.get("crm.carefor.lb.cycleTitle")xx $text.get('crm.carefor.lb.yearci')$text.get('common.validate.error.integer')");
			return false;
		}
	...
	// 具体事件 (0)，发短信(1)，发邮件(2)
	if(actionValue == 0){
		var con = form.eventContent.innerHTML;
...
<form method="post" name="form" action="/CareforDelAction.do" onSubmit="return val();" target="formFrame">
	...
	<input type="hidden" name="sequence" value="$bean.sequence">
	<input type="hidden" name="KeyId" value="$bean.ref_id">
	#if("true"=="$!detail")
	...
	<input type="hidden" name="ref_id" value="$bean.ref_id">
    ...
    <div class="oalistRange_scroll_1">
		<table border="0" width="100%" cellpadding="0" cellspacing="0" class="oalistRange_list_function" name="table">
			<tr>
			  ...
			  <td colspan="2"> &nbsp;
			    <input name="actionName" class="text2" value="$bean.actionName"  style="width:450px" type="text" /></td>
		  </tr>
		  <tr><td  height="5" colspan="3" valign="top" bgcolor="#D7EBFF"><strong>$text.get("crm.carefor.lb.execdate")</strong></td></tr>
			<tr>
			  ...
			  <td bgcolor="#F5F5F5"><input name="dateType" id="nodate"  #if($bean.dateType==0) checked="checked" #end type="radio"  value="0" checked="checked"/>$text.get("crm.carefor.lb.nodate")：</td>
			  <td>$text.get("crm.carefor.lb.nodatelimit")</td>
			</tr>
			<tr>
			  <td bgcolor="#F5F5F5"><input name="dateType"  #if($bean.dateType==1) checked="checked" #end type="radio" value="1" />$text.get("crm.carefor.lb.absolutedate")：</td>
		      <td>$text.get("common.lb.startDate")
			  <input type="text" name="startDate"  value="$bean.startDate" onClick="openInputDate(this)" class="text2" />$text.get("common.lb.endDate")<input type="text" name="endDate"  value="$bean.endDate" onClick="openInputDate(this)" class="text2"/></td>
		  </tr>
			<tr>
			  <td bgcolor="#F5F5F5"><input name="dateType"  #if($bean.dateType==2) checked="checked" #end type="radio" value="2" />$text.get("crm.carefor.lb.comparativedate")：</td>
		      <td>$text.get("crm.carefor.lb.afterdate")
			  <input type="text" name="baselineDate2" size="2"  #if($bean.dateType==2) value="$bean.baselineDate" #end class="text2"/>
			  ...
			  <input type="text" name="runDates2"  #if($bean.dateType==2) value="$bean.runDates" #end size="2" class="text2">$text.get("crm.carefor.lb.ci")</td>
		  </tr>
			<tr>
			  <td bgcolor="#F5F5F5"><input name="dateType"  #if($bean.dateType==3) checked="checked" #end type="radio" value="3" />$text.get("crm.carefor.lb.cycle")：</strong></td>
		      <td>$text.get("crm.carefor.lb.afterdate")
			  <input type="text" name="baselineDate3"  #if($bean.dateType==3) value="$bean.baselineDate" #end size="2" class="text2"/>
			  $text.get("crm.carefor.lb.afterdateover")，$text.get("crm.carefor.lb.erery")
			  <input type="text" name="space3"  #if($bean.dateType==3) value="$bean.space" #end size="2" class="text2">
			  $text.get("crm.carefor.lb.toexec")$text.get("crm.carefor.lb.repetition")
			  <input type="text" name="runDates3"  #if($bean.dateType==3) value="$bean.runDates" #end size="2" class="text2">
			  $text.get("crm.carefor.lb.ci")，$text.get("crm.carefor.lb.cycleTitle")
			  <input type="text" name="runTimes"  #if($bean.dateType==3) value="$bean.runTimes" #end size="2" class="text2">$text.get("crm.carefor.lb.yearci")</td>
		  </tr>
			<tr>
			...
			<tr>
			  <td align="right" valign="top" class="oabbs_tr"><input name="actionType"  #if($bean.actionType==0) checked="checked" #end type="radio" value="0" checked="checked">$text.get("crm.carefor.lb.actionconcretely")</td>
				
			  <td align="right" valign="top" bgcolor="#F5F5F5">
			 
			 $text.get("crm.carefor.lb.transactiondetach")：			  </td>
			  <td valign="top"> &nbsp;
		      <textarea name="eventContent"  cols="50" rows="6">$bean.eventContent</textarea>  ( $text.get("crm.carefor.lb.manualexec"))</td>
			</tr>
			<tr>
			  <td rowspan="2" align="right" valign="top" class="oabbs_tr"><input type="radio"  onclick="return setSMSOrEmail();" name="actionType"  #if($bean.actionType==1) checked="checked" #end value="1">$text.get("com.carefor.lb.sendsms")</td>
		      ...
		      &nbsp; $text.get("crm.carefor.lb.startcurday")&nbsp; <select name="smsTime">
					     #foreach ( $index in [0..23] )
					    	<option value="$index"  #if($bean.smsTime==$index)selected="selected" #end>$index:00</option>
					    #end
					  </select></td>
			</tr>
			<tr>
	          ...
	          <textarea name="smsContent" cols="50" rows="6">$bean.smsContent</textarea> </td>
		  </tr>
			<tr>
			 <td rowspan="4" align="right" valign="top"  class="oabbs_tr"><input type="radio"  onclick="return setSMSOrEmail();" name="actionType"  #if($bean.actionType==2) checked="checked" #end value="2">$text.get("common.lb.sendmail")</td>
			 <td align="right" valign="middle" bgcolor="#F5F5F5">$text.get("oa.mail.cc")：</td>
			 <td> &nbsp;
			   <input name="ccSelf" type="hidden" value="0">
          <input type="checkbox" name="cc"  #if($bean.ccSelf==1) checked="checked"  #end onClick="javascript:if(this.checked){form.ccSelf.value='1'} else {form.ccSelf.value='0'} ">
        $text.get("crm.carefor.lb.copyme")</td>
			</tr>
			<tr>
			  <td align="right" valign="middle" bgcolor="#F5F5F5">$text.get("oa.mail.emailUser")：</td>
		      <td> &nbsp;
		      <input type="radio" name="addressType" value="1" #if($!bean.addressType==1) checked="checked" #end/>
		      $text.get('crm.carefor.lb.compemail')(<a href="javascript:openSetting()">$text.get('common.msg.set')</a>) &nbsp;&nbsp;
		      <input type="radio" name="addressType" value="2"  #if($!bean.addressType==2) checked="checked" #end/>$text.get('crm.carefor.lb.exeemail')
		      </td>
		  </tr>
			<tr>
			  <td align="right" valign="middle" bgcolor="#F5F5F5">$text.get("crm.carefor.lb.emailtitle")：</td>
		      <td> &nbsp;
		        <input name="mailTitle" type="text" class="text" style="width: 500px;" value="$bean.mailTitle"/></td>
		  </tr>
			<tr>
			  <td align="right" valign="top" bgcolor="#F5F5F5">$text.get("oa.mail.context")：</td>
		      <td><textarea  name="mailContent" style="width:100%;height:300px;visibility:hidden;">$bean.mailContent</textarea>
	        </td>
		  </tr>
