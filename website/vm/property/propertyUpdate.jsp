...
addCols(editgriddata,'displays','$text.get("property.lb.name")',130,2,false,'',500,-1,va);
addCols(editgriddata,'values','$text.get("property.lb.values")',50,2,false,'',100,0,'');
addCols(editgriddata,'status','$text.get("property.lb.status")',100,2,false,'',100,1,IsAble);

#foreach($row in $result.propItem)
	addRows(editgriddata,'$row.handerDisplay()','$row.propValue');
#end
...
<div id="listRange_id">
	...
	<ul class="listRange_1">	
		<li><span>$text.get("property.lb.propName"):</span><input type="text" name="propName" style="width:80px" value="$result.propName"></li>
		<li><span>$text.get("property.lb.name"):</span><input type="text" name="maindisplay" style="width:50px" value="$result.handerDisplay()" onKeyPress="if(event.keyCode == 13){  mainmd();  }"></li>			
	</ul>
