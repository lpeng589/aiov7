...
(function($) {
	...
	function appendTreeNodes(setting, level, treeNodes, parentNode) {
		...
		for (var i = 0, l = treeNodes.length; i < l; i++) {
			...
			if(typeof(setting.isMenu)=="undefined" || setting.isMenu==null){
			...
			var strUrl = "" ;
			if(typeof(node.strUrl)!="undefined" && node.strUrl.length>0){
				strUrl = "isUrl='true'" ;
			}
			html.push("<a "+strUrl+" id='", node.tId, IDMark_A, "' treeNode", IDMark_A," onclick=\"", (node.click || ''),
	...
	function eventProxy(event) {
		...
		if (tools.eqs(event.type, "mouseover")) {
			...
			tmp = tools.getMDom(setting, target, [{tagName:"a", attrName:"treeNode"+IDMark_A}]);
			if (tmp) {tId = tmp.parentNode.id;}
		} else if (tools.eqs(event.type, "click") && typeof(setting.isMenu)=="undefined") {
		...
		} else if (tools.eqs(event.type, "dblclick")  || (typeof(setting.isMenu)!="undefined" && tools.eqs(event.type, "click"))) {
	...
	function makeNodeIcoStyle(setting, treeNode) {
		...
		if (!treeNode.isAjaxing) {
			...
			//次代码注释 否则覆盖zTreeStyle.css里面的根目录的background的显示 后台需要传icon对应icon路径
	...
	function addTreeDom(setting, treeNode) {
		if (setting.dragStatus == 0) {
			...
			if (setting.editable) {
				if(typeof(addModule) != 'undefined' && addModule == "true"){
					addNextBtn(setting, treeNode);
				}
				if(typeof(updateModule) != 'undefined' && updateModule == "true"){
					addEditBtn(setting, treeNode);
				}
				if(typeof(delModule) != 'undefined' && delModule == "true"){
	...
	//添加
	function addNextBtn(setting, treeNode) {
		if (!setting.edit_addBtn || $("#" + treeNode.tId + IDMark_Add).length > 0) {
			return;
		}
		if (!tools.apply(setting.edit_addBtn, [treeNode], setting.edit_addBtn)) {
			return;
		}
		var addObj = $("#" + treeNode.tId + IDMark_Span);
		var addStr = "<button type='button' class='add' id='" + treeNode.tId + IDMark_Add + "' title='添加下级' treeNode"+IDMark_Add+" onfocus='this.blur();' style='display:none;'></button>";
		addObj.after(addStr);
		
		$("#" + treeNode.tId + IDMark_Add).bind('click', 
			function() {
				//添加执行的函数（请不要顺便改名称，谢谢合作）
				addData(treeNode);
			}
		).show();
	}
	function addEditBtn(setting, treeNode) {
		if (treeNode.editNameStatus || $("#" + treeNode.tId + IDMark_Edit).length > 0 || treeNode.id=="0") {
			return;
		}
		if (!tools.apply(setting.edit_renameBtn, [treeNode], setting.edit_renameBtn)) {
			return;
		}
		var nObj = $("#" + treeNode.tId + IDMark_A);
		var editStr = "<button type='button' class='edit' id='" + treeNode.tId + IDMark_Edit + "' title='修改' treeNode"+IDMark_Edit+" onfocus='this.blur();' style='display:none;'></button>";
		nObj.append(editStr);
		
		$("#" + treeNode.tId + IDMark_Edit).bind('click', 
			function() {
				//修改执行的函数（请不要顺便改名称，谢谢合作）
				editData(treeNode);
			}
		).show();
	}
	function addRemoveBtn(setting, treeNode) {		
		if (!setting.edit_removeBtn || $("#" + treeNode.tId + IDMark_Remove).length > 0 || treeNode.id=="0") {
			return;
		}
		...
		var aObj = $("#" + treeNode.tId + IDMark_A);
		var removeStr = "<button type='button' class='remove' id='" + treeNode.tId + IDMark_Remove + "' title='删除' treeNode"+IDMark_Remove+" onfocus='this.blur();' style='display:none;'></button>";
		aObj.append(removeStr);
		
		$("#" + treeNode.tId + IDMark_Remove).bind('click', 
			function() {
				//删除执行的函数（请不要顺便改名称，谢谢合作）
				removeData(treeNode);
			}
			).bind('mousedown',
