...
public class AIOMimeUtility {

	// This class cannot be instantiated
	private AIOMimeUtility() {
	}

	public static final int ALL = -1;

	private static final boolean decodeStrict = PropUtil
			.getBooleanSystemProperty("mail.mime.decodetext.strict", true);

	private static final boolean encodeEolStrict = PropUtil
			.getBooleanSystemProperty("mail.mime.encodeeol.strict", false);
	 ...
	 */
	private static final boolean foldEncodedWords = PropUtil
			.getBooleanSystemProperty("mail.mime.foldencodedwords", false);

	private static final boolean foldText = PropUtil.getBooleanSystemProperty(
	 ...
	 *			conversion failed.
	 */
	public static String decodeText(String etext,boolean useCharset,String thecharset)
			throws UnsupportedEncodingException {
		/*
		...
		//未指定编码时---------------zxy
		if (etext.indexOf("=?x-unknown?") > -1) {
            // 将编码方式的信息由x-unkown改为gbk
			etext = etext.replaceAll("x-unknown", "gbk");
        }
		if (etext.toLowerCase().indexOf("=?gb2312?") > -1) {
            //gb2312的直接采用gbk来解码，因为gbk包含gb2312,而实践中发现不少邮件用gb2312编码，但实际采用的是gbk，如foxmail邮箱
			etext = "=?gbk?" +etext.substring("=?gb2312?".length());
        }
		if (useCharset && thecharset != null && thecharset.length() > 0) {
				etext = "=?"+thecharset+"?" +etext.substring(etext.indexOf("?",2)+1);
		}
	...
	private static void doEncode(String string, boolean b64, String jcharset,
			int avail, String prefix, boolean first, boolean encodingWord,
			StringBuffer buf) throws UnsupportedEncodingException {
		...
		if ((len > avail) && ((size = string.length()) > 1)) {
			...
			doEncode(string.substring(0, size / 2), b64, jcharset, avail,
					prefix, first, encodingWord, buf);
			doEncode(string.substring(size / 2, size), b64, jcharset, avail,
					prefix, false, encodingWord, buf);
		} else {
	...
	public static String decodeWord(String eword) throws ParseException,
		...
		if ((pos = eword.indexOf('?', start)) == -1)
			throw new ParseException("encoded word does not include charset: "
					+ eword);
		...
		start = pos + 1;
		if ((pos = eword.indexOf('?', start)) == -1)
			throw new ParseException("encoded word does not include encoding: "
					+ eword);
		...
		start = pos + 1;
		if ((pos = eword.indexOf("?=", start)) == -1)
			throw new ParseException("encoded word does not end with \"?=\": "
					+ eword);
		/*
		...
		try {
			...
			if (word.length() > 0) {
				...
				decodedWord = count <= 0 ? "" : new String(bytes, 0, count,
						charset);
			} else {
	...
	public static String javaCharset(String charset) {
		...
		String alias = (String) mime2java.get(charset
	...
	public static String mimeCharset(String charset) {
		...
		String alias = (String) java2mime.get(charset
				.toLowerCase(Locale.ENGLISH));
		return alias == null ? charset : alias;
	}
...
class AsciiOutputStream extends OutputStream {
	...
	private final void check(int b) throws IOException {
		...
		if (AIOMimeUtility.nonascii(b)) { // non-ascii
			non_ascii++;
			if (breakOnNonAscii) { // we are done
				ret = AIOMimeUtility.MOSTLY_NONASCII;
	...
	public int getAscii() {
		...
		// doesn't encode this case properly).
		if (badEOL)
			return AIOMimeUtility.MOSTLY_NONASCII;
		else if (non_ascii == 0) { // no non-us-ascii characters so far
			// if we've seen a long line, we degrade to mostly ascii
			if (longLine)
				return AIOMimeUtility.MOSTLY_ASCII;
			else
				return AIOMimeUtility.ALL_ASCII;
		}
		if (ascii > non_ascii) // mostly ascii
			return AIOMimeUtility.MOSTLY_ASCII;
		return AIOMimeUtility.MOSTLY_NONASCII;
