	...
	switch ( listenerName )
	{
		case 'Generic' :
			...
			AddItems : function( menu, tag, tagName )
			{
				...
				menu.AddItem( 'delObj',FCKLang.delObj,9,false);
			}} ;
		...
		case 'AIOCheckbox' :
			...
			AddItems : function(menu,tag,tagName)
			{
				if (tagName=='INPUT'&&tag.type=='checkbox' && tag.getAttribute('inputtype')=='AIOCheckbox')
				{
					menu.AddSeparator();
					menu.AddItem('AIOCheckbox',FCKLang.AIOCheckboxProp,97);
				}
			}};
		case 'Checkbox' :
			return {
		...
		case 'AIORadio' : 
			...
			AddItems : function(menu,tag,tagName)
			{
				if (tagName=='INPUT'&&tag.type=='radio'  && tag.getAttribute('inputtype')=='AIORadio')
				{
					menu.AddSeparator();
					menu.AddItem('AIORadio',FCKLang.AIORadioProp,98);
					}
				}};
		case 'Radio' :
			return {
		...
		case 'AIOTextField' :
			...
			AddItems : function(menu,tag,tagName)
			{ 
				if (tagName=='INPUT' && tag.getAttribute('inputtype')=='AIOText')
				{
					menu.AddSeparator();
					menu.AddItem('AIOTextField',FCKLang.AIOTextFieldProp,92);
				}
			}};
		case 'TextField' :
			return {
			AddItems : function( menu, tag, tagName )
			{
				if ( tagName == 'INPUT' && ( tag.type == 'text' || tag.type == 'password' ) && tag.getAttribute('inputtype')!='AIOText' )
				{
					menu.AddSeparator() ;
		...
		case 'AIOSelect' :
			...
			AddItems : function(menu,tag,tagName)
			{
				if (tagName=='SELECT' && tag.getAttribute('inputtype')=='AIOSelect')
				{
					menu.AddSeparator();
					menu.AddItem('AIOSelect',FCKLang.AIOSelectionFieldProp,95);
				}
			}};
		case 'Select' :
			return {
		...
		case 'AIOTextarea' : 
			...
			AddItems : function(menu,tag,tagName)
			{
				if (tagName=='TEXTAREA' && tag.getAttribute('inputtype')=='AIOTextarea')
				{
					menu.AddSeparator();
					menu.AddItem('AIOTextarea',FCKLang.AIOTextareaProp,94);
				}
			}};
		case 'AIODetail' : 
			return {
			AddItems : function(menu,tag,tagName)
			{
				if (tagName=='TEXTAREA' && tag.getAttribute('inputtype')=='AIODetail')
				{
					menu.AddSeparator();
					menu.AddItem('AIODetail',FCKLang.AIODetailProp,94);
				}
			}};	
		case 'Textarea' :
			return {
			AddItems : function( menu, tag, tagName )
			{
				if ( tagName == 'TEXTAREA'  && tag.getAttribute('inputtype')!='AIOTextarea' && tag.getAttribute('inputtype')!='AIODetail' )
				{
					menu.AddSeparator() ;
