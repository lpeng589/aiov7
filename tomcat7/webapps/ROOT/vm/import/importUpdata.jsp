...
function downModule(){
	if("$importName"=="tblIniAccDet"){
		jQuery.ajax({type:"post",url:"/IniAccQueryAction.do?operation=7&opType=downModule&accNumber=$!moduleParam",success: function(msg){window.location.href=msg;}});
	}else{
		mdiwin('/UserFunctionQueryAction.do?operation=25&tableName=$importName&ButtonType=billExport&example=true&moduleType=$!moduleType&noback=true','$text.get("excel.bill.import")');
	}
}
function keyDown(e) { 
...
<input type="hidden" name="importName" value="$importName"/>
<input type="hidden" name="moduleType" value="$!moduleType"/>
<input type="hidden" name="moduleParam" value="$!moduleParam"/>
...
<input type="hidden" name="fromPage" value="$!fromPage">
		<table cellpadding="0" cellspacing="0" class="framework">
			<tr>
				<td>
					<div class="TopTitle">
						<span>$text.get("excel.list.updata") &nbsp;--&nbsp; [&nbsp;$!globals.getTableDisplayName($importName)&nbsp;]</span>
								...
								<div class="inputbox">
										...
										<ul>
											...
											<div style="width: 100%;">
												<div style="float: left;margin-left: 40px;margin-right:5px; padding-top:2px; vertical-align: middle;">导入模板: </div>
													...
													<!-- <div class="lp"  id="$view.id"  style="float: left;"  onclick="downView('/UserFunctionQueryAction.do?operation=25&tableName=$importName&ButtonType=billExport&example=true&moduleType=$!importData.moduleType&noback=true');"><span class="a"></span>点击下载<span class="c"></span></div>-->
							        		 		<div class="lp"  id="$view.id"  style="float: left;"  onclick="downModule()"><span class="a"></span>点击下载<span class="c"></span></div>
											</div>
									...
									<div>
										...
										<ul id="showRs" style="display: none;">		
											<li>
												<span>$text.get("aio.import.total"):<font style="color: red;">$!totalimport</font></span></div>					
											</li>
											<li>
												<span>$text.get("import.succees.number"):<font style="color: red;">$!successimport</font></span></div>					
											</li>
											<li>
												<span>$text.get("import.failure.number"):<font style="color: red;">$!errorimport</font></span></div>				
											</li>
											#if("$!errorimport" != "0")
