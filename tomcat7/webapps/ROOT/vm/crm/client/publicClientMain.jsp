...
<input type="hidden" id="public" name="public" value="public">
#foreach($define in $defineList)
#if("2"=="$globals.get($define,4)")
<input type="hidden" name="$globals.get($define,1)" value="$!conMap.get($globals.get($define,1))">
#end
#end 
<div class="oamainhead">
	<ul style="float: left; margin-left:4px;margin-top:3px;">
		<li style="float:left">$text.get("common.lb.key")：
			...
			&nbsp;<button onclick="beforeSubmit();" style="margin-bottom:5px;">查询</button>
		</li>
	</ul>
	<div class="HeadingButton">
		...
		<li><button  onClick="javascript:telCall()" class="b2">$text.get("common.lb.telCall")</button></li>
		#end
		...
		#end
		#if($LoginBean.operationMap.get("/EMailAction.do").add())
			...
			<div class="listRange_3" id="listid">
					...
					#foreach($define in $defineList)
							...
							<li><p>$text.get("oa.common.department")</p>：
								...
								<input type="text" name="deptName" ondblclick="selectDept();" readonly value="$!conMap.get("deptName")" style="width: 120px;"/>
				...
				<tbody>
					...
					<tr onclick="setIframSrc2('/CrmTabAction.do?tableName=CRMClientInfo&tabType=detail&keyId=$!globals.get($!client,0)',this)" onMouseMove="setBackground(this,true);" onMouseOut="setBackground(this,false);">
						...
						<td width="60" class="oabbs_tc">
							<a href="javascript:mdiwin('/CrmTabAction.do?operation=5&keyId=$!globals.get($!client,0)&public=public','$text.get("common.lb.clientDetail")')";>$text.get("sms.note.detail")</a>
						</td>
					</tr>
					#end
				</tbody>
...
<input type="hidden" name="strAction" value="/ClientAction.do?operation=4&type=main&public=$public"/>
