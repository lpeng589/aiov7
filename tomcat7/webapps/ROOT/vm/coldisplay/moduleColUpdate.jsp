...
var No = 0;
function addTr(varTableName,varTableDis,varFieldName,varFieldDis,varType,varLanguage){
	...
	fieldLanguage.innerHTML =  "<input type=text name=\"fieldLanague\" readonly ondblclick='moreLanguage(\"$globals.getLocale()\",this)' value=\""+varLanguage+"\"/>";
...
function init(){
	#foreach($popup in $popupList)
		#if("$popup.fieldType"=="bill")
			addTr('$popup.tableName','$globals.getTableDisplayName($popup.tableName)','${popup.fieldName}-${popup.popupFieldName}-${popup.popupName}-bill','$globals.getFieldDisplay($popup.tableName,${popup.fieldName})-$globals.getFieldDisplay(${popup.popupFieldName})','$popup.fieldType','$popup.language') ;
		#else
			addTr('$popup.tableName','$popup.moduleDisplay','${popup.fieldName}-${popup.popupFieldName}-${popup.popupName}-list','$globals.getFieldDisplay(${popup.fieldName})-$globals.getFieldDisplay(${popup.popupFieldName})','$popup.fieldType','$popup.language') ;
		#end
	#end
}
</script>
...
<input type="hidden" name="moduleId" value="$popupList.get(0).moduleId">
<input type="hidden" name="moduleName" value="$popupList.get(0).moduleName">
<input type="hidden" name="winCurIndex" value="$!winCurIndex">
<div class="Heading">
	<div class="HeadingIcon"><img src="/$globals.getStylePath()/images/Left/Icon_1.gif"></div>
	<div class="HeadingTitle">$text.get("com.update.module.name")</div>
	<ul class="HeadingButton">
		  ...
		  <li>
		  <button type="button" onClick="location.href='/ModuleColAction.do?operation=$globals.getOP("OP_QUERY")&winCurIndex=$!winCurIndex'" class="b2">$text.get("common.lb.back")</button></li>
	</ul>
</div>
...
<div id="listRange">
		<div class="listRange_1">
			<li><span> $text.get("com.module.name")：</span>
				<input type="text" id="moduleDisplay" name="moduleDisplay" onClick="selectModuleName();" value="$popupList.get(0).moduleDisplay" style="50px; background:url(/$globals.getStylePath()/images/St.gif) no-repeat bottom right;cursor:pointer" readonly="readonly">
			</li>
			<li>
		...
		<script type="text/javascript">
			...
			oDiv.style.height=sHeight+"px";
				...
				<table id="myTable" width="730" border="0" cellpadding="0" cellspacing="0" class="listRange_list">
				<thead>
