...
$.TextboxListBit = function(type, value, textboxlist, _options){
	...
	var init = function(){
		...
		if (type == 'box'){
			//fjj
			var val = chk(self.value[1]) ? self.value[1] : self.value[0];
			val = val.replace("<","&lt;").replace(">","&gt;");
			
			bit.html(val).click(focus);
			if (options.deleteButton){
				//在显示的数据中class加入class='showSelect'
				bit.addClass(typeprefix + '-deletable showSelect');
				//加了一个class='boxSelect' 方便页面循环获取值
				close = $('<a href="#" id="'+self.value[0]+'" class="'+ typeprefix +'-deletebutton boxSelect" />').click(focus).click(remove).appendTo(bit);
			}
			bit.children().click(function(e){ e.stopPropagation(); e.preventDefault(); });
		} else {
			element = $('<input type="text" class="inp_obj inp-txt '+ typeprefix +'-input" autocomplete="off" />').val(self.value ? self.value[1] : '').appendTo(bit);
			...
			element.focus(function(){ focus(true); }).blur(function(){
				...
				var obj_input = textboxlist.getElement.attr("ob_type");
			  	var v = element.val();
			  	if(typeof(obj_input)!=undefined && obj_input == "mail" && v!=""){
			  		if(isMail(v)){
			  			//验证是否是email,如果是就创建一个textBox框
				  		toBox();
			  		}
			  	}else if(typeof(obj_input)!=undefined && obj_input == "text" && v!=""){
			  		toBox();
			  	}
