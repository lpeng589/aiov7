...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblGoods&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblGoods&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblGoods&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblGoods&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblGoodsStockLimit"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="tblGoodsUnit"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblGoodsStockLimit"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblGoodsUnit"){
 
 }
 }
}

 var tblName=new Array(2);
 
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblGoodsStockLimit"; 
 tblName[1]="tblGoodsUnit"; 
...
function openAttachUpload(type,btnId){
 ...
 ' <param name="movie" value="/flash/FileUpload.swf" /><param name="quality" value="high" /><param name="bgcolor" value="#869ca7" /><param name="flashvars" value="maxSize=2&uploadUrl=/UploadServlet;jsessionid='+aioSessionId+'?path=/temp/@amp;fieldName='+btnId+'@amp;fileType='+type+'&filter='+filter+'&btnId='+btnId+'" />'+
 ...
 ' flashvars="maxSize=2&uploadUrl=/UploadServlet;jsessionid='+aioSessionId+'?path=/temp/@amp;fieldName='+btnId+'@amp;fileType='+type+'&filter='+filter+'&btnId='+btnId+'" quality="high" allowScriptAccess="sameDomain" type="application/x-shockwave-flash" pluginspage="http/'+'/www.adobe.com/go/getflashplayer"></embed></object>';
} 
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblGoods\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblGoods\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'upperQty',dis:'库存上限',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'lowerQty',dis:'库存下限',digit:2,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'ProSalesPrice',dis:'预设售价',digit:2,isMain:true};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblGoods",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblGoods&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblGoods&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblGoods&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblGoods&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='tblGoodsStockLimit';
tabIsNulls[childCount]='0';
tabDiss[childCount]='库存上下限明细表';
childCount++;
 var tblGoodsStockLimitData = new gridData();
 gridDatas[gridDataNum] = tblGoodsStockLimitData;
 gridDataNum++;
 var tblGoodsStockLimit=1;
 ...
 addCols(tblGoodsStockLimitData,'tblGoodsStockLimit_StockCode','仓库名称','150',2,false,'',150,-2,'','',"","","2",'',"2",true);
 putChildValidateItem("tblGoodsStockLimit_StockCode","仓库名称","any","",false,minValue,150,false,'');
 
 
 addCols(tblGoodsStockLimitData,'tblGoodsStockLimit_tblStock_StockFullName1','仓库名称','75',0,true,'',1000000,2,'tblGoodsStockLimit:StockCode:tblGoodsStockLimit_tblStock_StockFullName1:'+param4,':',"","","2",'',"2",false); 
 
 
 
 addCols(tblGoodsStockLimitData,'tblGoodsStockLimit_UpperLimit','库存上限','100',0,true,'0',0,0,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblGoodsStockLimit_UpperLimit","库存上限","int","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblGoodsStockLimitData,'tblGoodsStockLimit_LowerLimit','库存下限','100',0,true,'0',0,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblGoodsStockLimit_LowerLimit","库存下限","int","",true,minValue,maxValue,true,'0',true); 
 
 
 
 addCols(tblGoodsStockLimitData,'tblGoodsStockLimit_BatchNo','批号','120',2,true,'',100,-2,'','',"","triggerCopy","5",'',"0",false); 
 putChildValidateItem("tblGoodsStockLimit_BatchNo","批号","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblGoodsStockLimitData,'tblGoodsStockLimit_Inch','厚度','70',2,true,'',100,-2,'','',"","triggerCopy","6",'',"0",false); 
 putChildValidateItem("tblGoodsStockLimit_Inch","厚度","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblGoodsStockLimitData,'tblGoodsStockLimit_Hue','颜色','70',2,true,'',100,-2,'','',"","triggerCopy","7",'',"2",false); 
 putChildValidateItem("tblGoodsStockLimit_Hue","颜色","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblGoodsStockLimitData,'tblGoodsStockLimit_yearNO','尺码','70',2,true,'',100,-2,'','',"","triggerCopy","8",'',"2",false); 
 putChildValidateItem("tblGoodsStockLimit_yearNO","尺码","any","GoodsField",true,minValue,100,false,''); 
 
 
 
 addCols(tblGoodsStockLimitData,'tblGoodsStockLimit_ProDate','生产日期','80',5,true,'',10,-2,'','',"","","9",'',"0",false); 
 putChildValidateItem("tblGoodsStockLimit_ProDate","生产日期","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 addCols(tblGoodsStockLimitData,'tblGoodsStockLimit_Availably','保质期限','80',5,true,'',10,-2,'','',"","","10",'',"0",false); 
 putChildValidateItem("tblGoodsStockLimit_Availably","保质期限","date","GoodsField",true,minValue,10,false,''); 
 
 
 
 
 addCols(tblGoodsStockLimitData,'tblGoodsStockLimit_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblGoodsUnit';
tabIsNulls[childCount]='0';
tabDiss[childCount]='商品辅助单位';
childCount++;
 var tblGoodsUnitData = new gridData();
 gridDatas[gridDataNum] = tblGoodsUnitData;
 gridDataNum++;
 var tblGoodsUnit=1;
 ...
 addCols(tblGoodsUnitData,'tblGoodsUnit_Result','换算关系','80',2,true,'1 AssistantUnit=? BaseUnit',100,-2,'','',"","","3",'',"0",false); 
 putChildValidateItem("tblGoodsUnit_Result","换算关系","any","",true,minValue,100,true,'1 AssistantUnit=? BaseUnit'); 
 
 
 
 addCols(tblGoodsUnitData,'tblGoodsUnit_DefaultAssit','默认辅助单位','75',2,true,'2',30,1,';1:是;2:否','',"","","3",'',"1",false);
 putChildValidateItem("tblGoodsUnit_DefaultAssit","默认辅助单位","any","",true,minValue,30,true,'2',true); 
 
 
 
 addCols(tblGoodsUnitData,'tblGoodsUnit_UnitD','基本单位比值','100',0,true,'1',0,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("tblGoodsUnit_UnitD","基本单位比值","int","",true,minValue,maxValue,true,'1',true); 
 
 
 
 addCols(tblGoodsUnitData,'tblGoodsUnit_SecUnitD','辅助单位比值','100',0,true,'1',0,0,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblGoodsUnit_SecUnitD","辅助单位比值","int","",true,minValue,maxValue,true,'1',true); 
 ...
 addCols(tblGoodsUnitData,'tblGoodsUnit_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblGoods&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=000020000400004&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblGoods,tblGoodsStockLimit,tblGoodsUnit,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblGoods&allTableName=tblGoods,tblGoodsStockLimit,tblGoodsUnit,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblGoods&allTableName=tblGoodsStockLimit,tblGoodsUnit,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblGoods&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblGoods&allTableName=tblGoodsStockLimit,tblGoodsUnit,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblGoods&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblGoods&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblGoods&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblGoods&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblGoods&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '商品';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblGoods&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
