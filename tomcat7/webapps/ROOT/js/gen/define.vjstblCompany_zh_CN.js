...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblCompany&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblCompany&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblCompany&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblCompany&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblCompanyEmployeeDet"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblCompanyEmployeeDet"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblCompanyEmployeeDet"; 
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblCompany\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblCompany\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'Credit',dis:'信用额度',digit:0,isMain:true};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblCompany",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblCompany&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblCompany&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblCompany"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblCompany&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblCompany&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
childData[childCount]='tblCompanyEmployeeDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='往来单位联系人';
childCount++;
 var tblCompanyEmployeeDetData = new gridData();
 gridDatas[gridDataNum] = tblCompanyEmployeeDetData;
 gridDataNum++;
 var tblCompanyEmployeeDet=1;
 
 
 addCols(tblCompanyEmployeeDetData,'tblCompanyEmployeeDet_contactCode','联系人编号','80',2,true,'',100,-2,'','',"","","0",'',"0",false); 
 putChildValidateItem("tblCompanyEmployeeDet_contactCode","联系人编号","any","",true,minValue,100,false,''); 
 ...
 addCols(tblCompanyEmployeeDetData,'tblCompanyEmployeeDet_ContactMan','联系人','100',2,true,'',50,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblCompanyEmployeeDet_ContactMan","联系人","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(tblCompanyEmployeeDetData,'tblCompanyEmployeeDet_Role','角色','90',2,true,'',50,1,';11:职员;1:法人;10:董事;9:董事长;2:总经理;3:校长;5:会计;4:主管;6:出纳;7:前台文员;8:业务员;12:其他','',"","","2",'',"1",false);
 putChildValidateItem("tblCompanyEmployeeDet_Role","角色","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(tblCompanyEmployeeDetData,'tblCompanyEmployeeDet_Gender','性别','50',0,true,'',0,1,';1:男;2:女','',"","","3",'',"1",false);
 putChildValidateItem("tblCompanyEmployeeDet_Gender","性别","int","",true,minValue,maxValue,false,'',true); 
 
 
 
 addCols(tblCompanyEmployeeDetData,'tblCompanyEmployeeDet_BirthDay','生日','80',5,true,'',10,0,'','',"","","4",'',"0",false); 
 putChildValidateItem("tblCompanyEmployeeDet_BirthDay","生日","date","",true,minValue,10,false,'',true); 
 
 
 
 addCols(tblCompanyEmployeeDetData,'tblCompanyEmployeeDet_Mobile','手机','100',2,true,'',50,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("tblCompanyEmployeeDet_Mobile","手机","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(tblCompanyEmployeeDetData,'tblCompanyEmployeeDet_Telephone','电话','100',2,true,'',50,0,'','',"","","5",'',"0",false); 
 putChildValidateItem("tblCompanyEmployeeDet_Telephone","电话","any","",true,minValue,50,false,'',true); 
 
 
 
 addCols(tblCompanyEmployeeDetData,'tblCompanyEmployeeDet_ClientEmail','Email','100',8,true,'',50,0,'','',"","","6",'',"0",false); 
 putChildValidateItem("tblCompanyEmployeeDet_ClientEmail","Email","mail","",true,minValue,50,false,'',true); 
 
 
 
 addCols(tblCompanyEmployeeDetData,'tblCompanyEmployeeDet_MSN','MSN','100',2,true,'',50,-2,'','',"","","7",'',"0",false); 
 putChildValidateItem("tblCompanyEmployeeDet_MSN","MSN","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblCompanyEmployeeDetData,'tblCompanyEmployeeDet_QQ','QQ','100',2,true,'',50,-2,'','',"","","8",'',"0",false); 
 putChildValidateItem("tblCompanyEmployeeDet_QQ","QQ","any","",true,minValue,50,false,''); 
 
 
 
 addCols(tblCompanyEmployeeDetData,'tblCompanyEmployeeDet_Hobby','爱好','120',2,true,'',500,-2,'','',"","","10",'',"0",false); 
 putChildValidateItem("tblCompanyEmployeeDet_Hobby","爱好","any","",true,minValue,500,false,''); 
 
 
 
 addCols(tblCompanyEmployeeDetData,'tblCompanyEmployeeDet_Remark','备注','120',2,true,'',8000,0,'','',"","","11",'',"0",false); 
 putChildValidateItem("tblCompanyEmployeeDet_Remark","备注","any","",true,minValue,8000,false,'',true); 
 
 
 
 addCols(tblCompanyEmployeeDetData,'tblCompanyEmployeeDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblCompany&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=0000200001&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblCompany,tblCompanyEmployeeDet,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblCompany&allTableName=tblCompany,tblCompanyEmployeeDet,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblCompany&allTableName=tblCompanyEmployeeDet,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblCompany&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblCompany&allTableName=tblCompanyEmployeeDet,&colType=colWidth" ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblCompany&designId=&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblCompany&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblCompany&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblCompany&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '客户';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblCompany&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
