...
function auditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=submit&tableName=tblexpenseaccount&workFlowNode="+workFlowNode,
...
function backAuditing(){
 var varReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=back&keyId="+valueId+"&tableName=tblexpenseaccount&workFlowNode="+workFlowNode,
 winObj,"dialogWidth:600px;dialogHeight:400px;center:yes;help:no;resizable:no;status:no;scroll:no;");
 if(typeof(varReturn)=="undefined")return false ;
 var strFile = varReturn.split("||") ;
 m("varWakeUp").value = strFile[0] ;
...
function auditLogList(){
 var file = window.showModalDialog("/UserFunctionAction.do?tableName=tblexpenseaccount&keyId="+valueId+"&checkTab=Y&optionType=Auditing&type=audtiting&operation=5&fromPage=yes",
...
function selectReadPerson(){
 var varPerson = window.showModalDialog("/vm/oa/fileflow/selectReadPerson.jsp?keyId="+valueId+"&tableName=tblexpenseaccount&wakeup="+strWakeUp,
 winObj,"dialogWidth:700px;dialogHeight:300px;center:yes;help:no;resizable:no;status:no;scroll:no;") ;
}
function newSave()
{ 
...
function copyFields2(index,order,tableName,copyAll){
 if(index==0)return ;
 var copyFields;
 if(typeof(tableName)!="undefined" && tableName=="tblexpenseaccountDet"){
 
 }
 if(typeof(tableName)!="undefined" && tableName=="tblexpenseaccountDet1"){
...
if(trigType=="dbClick" && index>0){
 var copyFields;
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblexpenseaccountDet"){
 
 }
 if(typeof(curGridRowNum.curKey)!="undefined" && curGridRowNum.curKey=="tblexpenseaccountDet1"){
...
var goodsCodeLogicValidate =new Array();
 tblName[0]="tblexpenseaccountDet"; 
 tblName[1]="tblexpenseaccountDet1"; 
...
function execStat2(){
 execStatOne('tblexpenseaccountDet_Business',2);
...
function onCompleteUpload(retstr,btnId){ 
 ...
 if(str == "") continue;
 var ul=$('#'+buttonId);
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div class='showAffix'>"+str+"</div><a class='showAffixDel' href='javascript:deleteupload(\""+str+"\",\"true\",\"tblexpenseaccount\",\"AFFIX\");'>删除</a><a class='showAffixDown' href=\"/ReadFile?fileName="+encodeURI(str)+"&realName="+encodeURI(str)+"&tempFile=true&type=AFFIX\" target=_blank>下载</a></li>";
 ul.append(imgstr);
 }
 } 
 var attachUpload = document.getElementById("attachUpload");
...
function linkPIC(btnId){
 ...
 　　　if(action == 'ok'){
 　　　　　str = val;
 var imgstr = "<li style='background:url();' id='uploadfile_"+str+"'><input type=hidden name="+hiddenName+" value='"+str+"'><div><a style=\"cursor:pointer;\" href='javascript:void(0);' onclick='deleteupload(\""+str+"\",\"false\",\"tblexpenseaccount\",\"PIC\");'>删除</a><em>"+str+"</em></div><div><a href=\""+str+"\" target=\"_blank\"><img src=\""+str+"\" height=\"120\" border=\"0\"></a></div></li>";
 ul.append(imgstr);
 　　　}
 }); 
}
...
{
 delFireEventOne(rowId,tableName,'tblexpenseaccountDet_Business',"tblexpenseaccountDet"); 
 }

var fieldDigit = new Array();
 fieldDigit[fieldDigit.length] = {name:'originalBorrow',dis:'原借款',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'backBlance',dis:'应退余款',digit:0,isMain:true};
 fieldDigit[fieldDigit.length] = {name:'tblexpenseaccountDet_Business',dis:'金额(元)',digit:2,isMain:false};
 fieldDigit[fieldDigit.length] = {name:'tblexpenseaccountDet1_Business',dis:'金额(元)',digit:0,isMain:false};
...
function beforSubmit(form){ 
 ...
 var strReturn = window.showModalDialog("/OAFileWorkFlowAction.do?operation=94&type=add&tableName=tblexpenseaccount",
...
function printData(billId,reportNumber){ 
 if(printRight){
 if(confirm("是否打印单据?")){
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblexpenseaccount&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&printType=savePrint&winCurIndex="+winCurIndex ;
...
function prints(billId,reportNumber){
 ...
 billId2=billId
 var strUrl = "/UserFunctionQueryAction.do?tableName=tblexpenseaccount&reportNumber="+reportNumber+"&moduleType="+moduleType+"&operation=18&BillID="+billId+"&parentTableName=&winCurIndex="+winCurIndex;
 asyncbox.open({id:'printPopup',title:'科荣控件',url:strUrl,width:300,height:280});
 enableForm(form);
}
...
function dyGetCalculate(url,obj){
 ...
 fieldName = "tblexpenseaccount"+"_"+obj.name ;
 }
...
function sendBillMsg(action){
 ...
 var strurl = "/vm/classFunction/sendBillMsg.jsp?tableName=tblexpenseaccount&action="+action
...
function telCall()
{
 var str = window.showModalDialog("/TelAction.do?operator=callTel&from=tblexpenseaccount&keyId="+valueId,"","dialogWidth=600px;dialogHeight=350px"); 
}
...
function initMainCaculate(){
 var z=n('originalBorrow');
 for(i = 0;i<z.length;i++){ 
 z[i].onchange = function(){ 
 n('backBlance')[0].value=f(" ",2);

if(nn('originalBorrow',0)>=0.01)n('backBlance')[0].value=f(Number(nn('originalBorrow',0))-Number(sm('tblexpenseaccountDet_Business')),2);
...
function initCalculate(){
 ...
 for(i = 0;i<z.length;i++){
 z[i].onchange = function(){ 
 var p =getrp(this,'tblexpenseaccountDet_Business'); 
 n('backBlance')[0].value=f(" ",2);
if(nn('originalBorrow',0)>=0.01)n('backBlance')[0].value=f(Number(nn('originalBorrow',0))-Number(sm('tblexpenseaccountDet_Business')),2);
...
childData[childCount]='tblexpenseaccountDet';
tabIsNulls[childCount]='0';
tabDiss[childCount]='费用报销明细';
childCount++;
 var tblexpenseaccountDetData = new gridData();
 gridDatas[gridDataNum] = tblexpenseaccountDetData;
 gridDataNum++;
 var tblexpenseaccountDet=4;
 ...
 addCols(tblexpenseaccountDetData,'tblexpenseaccountDet_EmployeeCode','用途','350',2,false,'',100,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblexpenseaccountDet_EmployeeCode","用途","any","",false,minValue,100,false,'',true); 
 
 
 
 addCols(tblexpenseaccountDetData,'tblexpenseaccountDet_Business','金额(元)','60',1,false,'',0,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblexpenseaccountDet_Business","金额(元)","double","",false,minValue,maxValue,false,'',true); 
 
 
 addCols(tblexpenseaccountDetData,'tblexpenseaccountDet_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
 
childData[childCount]='tblexpenseaccountDet1';
tabIsNulls[childCount]='0';
tabDiss[childCount]='费用报销明细';
childCount++;
 var tblexpenseaccountDet1Data = new gridData();
 gridDatas[gridDataNum] = tblexpenseaccountDet1Data;
 gridDataNum++;
 var tblexpenseaccountDet1=1;
 
 
 
 
 addCols(tblexpenseaccountDet1Data,'tblexpenseaccountDet1_EmployeeCode','用途','350',2,false,'',100,0,'','',"","","1",'',"0",false); 
 putChildValidateItem("tblexpenseaccountDet1_EmployeeCode","用途","any","",false,minValue,100,false,'',true); 
 
 
 
 addCols(tblexpenseaccountDet1Data,'tblexpenseaccountDet1_Business','金额(元)','60',1,false,'',0,0,'','',"","","2",'',"0",false); 
 putChildValidateItem("tblexpenseaccountDet1_Business","金额(元)","double","",false,minValue,maxValue,false,'',true); 
 
 
 addCols(tblexpenseaccountDet1Data,'tblexpenseaccountDet1_detsId','明细ID','0',2,true,'',100,-2,'','',"","","",'','3'); 
...
function showDivCustomSetTable(){ 
 ...
 var strUrl = "/UserFunctionAction.do?tableName=tblexpenseaccount&moduleType="+moduleType+"&winCurIndex="+winCurIndex+"&f_brother=&noback="+noback+"&parentCode=&operation=6&parentTableName=&configType=colConfig" ;
...
function fillColConfig(returnValue){
 ...
 var tables = "tblexpenseaccount,tblexpenseaccountDet,tblexpenseaccountDet1,".split(",") ;
 ...
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblexpenseaccount&allTableName=tblexpenseaccount,tblexpenseaccountDet,tblexpenseaccountDet1,&colType=bill";
 jQuery.ajax({url: strURL,type: "POST",data:"colSelect="+strCols,async: false}); 
 document.location.href = document.location.href ;
 return;
 }else if("defaultConfig"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblexpenseaccount&allTableName=tblexpenseaccountDet,tblexpenseaccountDet1,&colType=bill" ;
 }else if("colWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=colconfig&tableName=tblexpenseaccount&colType=colWidth&colSelect="+strType[1] ;
 }else if("defaultWidth"==strType[0]){
 var strURL = "/UtilServlet?operation=defaultConfig&tableName=tblexpenseaccount&allTableName=tblexpenseaccountDet,tblexpenseaccountDet1,&colType=colWidth" ;
...
function extendSubmit(vars,vname,selected)
{
 ...
 var varValue = window.showModalDialog("/vm/oa/oaPublicMsg/folderRight.jsp?tableName=tblexpenseaccount&keyId="+keyIdStr,"","dialogWidth=565px;dialogHeight=350px") ;
 if(typeof(varValue)=="undefined")return ;
...
function update(){ 
 location.href="/UserFunctionAction.do?tableName=tblexpenseaccount&designId=12bbe4d0_1309251420149973794&keyId="+$("#id").val()+
...
{ 
 form.target = ""; 
 form.action="/UserFunctionAction.do?&operation=17&tableName=tblexpenseaccount&saveDraft=&&keyId="+keyId+
...
function checkSave(str){ 
 ...
 jQuery.post("/UserFunctionQueryAction.do?tableName=tblexpenseaccount&operation=25&ButtonType=saveCheckBill&varKeyIds="+cid+defi+"&moduleType="+moduleType,
 {ButtonTypeName:'审核保存'},
 function(data){
 if(data=="OK"){
 checkDialog("/OAMyWorkFlow.do?keyId="+cid+"&tableName=tblexpenseaccount&operation=82&fromPage=erp");
...
function hurryTrans(){
 ren = '费用报销单';
 var url="/vm/oa/workflow/noteModel.jsp?tableName=tblexpenseaccount&moduleName="+encodeURI(ren)+"&keyId="+$("#id").val();
 asyncbox.open({ 
